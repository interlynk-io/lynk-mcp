...
func (s *Server) registerTools() {
	...
	s.mcp.AddTool(mcp.NewTool("download_sbom",
		mcp.WithDescription("Download an SBOM with configurable format/options. Provide version_id directly, or product_name/product_id plus optional environment to download the latest matching version."),
		mcp.WithString("version_id", mcp.Description("The UUID of the version/SBOM to download. If omitted, product lookup is used.")),
		mcp.WithString("product_id", mcp.Description("Product/repository UUID for latest-version lookup")),
		mcp.WithString("product_name", mcp.Description("Exact product/repository name for latest-version lookup")),
		mcp.WithString("environment_id", mcp.Description("Environment/project UUID for latest-version lookup")),
		mcp.WithString("environment_name", mcp.Description("Exact environment/project name for latest-version lookup. Defaults to production when present.")),
		mcp.WithString("version", mcp.Description("Optional exact version string; omitted means latest version in the environment")),
		mcp.WithString("spec", mcp.Description("SBOM spec: CycloneDX or SPDX")),
		mcp.WithString("spec_version", mcp.Description("SBOM spec version, e.g. 1.6 or 2.3")),
		mcp.WithBoolean("include_vulns", mcp.Description("Include vulnerabilities in the SBOM")),
		mcp.WithBoolean("include_files", mcp.Description("Include file artifacts in the export")),
		mcp.WithBoolean("lite", mcp.Description("Generate a lite SBOM")),
		mcp.WithBoolean("dont_package_sbom", mcp.Description("Return raw content instead of base64-packaged content")),
		mcp.WithBoolean("original", mcp.Description("Download the original uploaded SBOM")),
		mcp.WithBoolean("exclude_parts", mcp.Description("Exclude parts from the export")),
		mcp.WithBoolean("include_support_status", mcp.Description("Include support status in the export")),
		mcp.WithBoolean("support_level_only", mcp.Description("Download support-level CSV instead of an SBOM")),
		mcp.WithBoolean("redact_internal_components", mcp.Description("Redact internal components")),
		mcp.WithString("tlp_classification_override", mcp.Description("Override TLP classification: CLEAR, GREEN, AMBER, AMBER_AND_STRICT, or RED")),
		mcp.WithArray("require_completed", mcp.Description("Processing stages that must be finished before content is returned: AUTOMATION, VULN_SCAN, POLICY_SCAN"), mcp.WithStringItems()),
		mcp.WithNumber("latest_version_limit", mcp.Description("Maximum versions to inspect when resolving latest (default: 100)")),
		mcp.WithBoolean("include_content", mcp.Description("Include download content in the response (default: true)")),
	), s.handleDownloadSBOM)
